...
const Cart = () => {
  ...
  return (
    <div className={`bg-blurBackground text-black fixed w-full h-full left-0 top-0 z-10 justify-end flex`}>
      <div className={`${isShowCart ? "w-96" : "w-0"} transition-all duration-200 ease-in-out h-full bg-white rounded-t-2xl mt-5 flex flex-col`}>
        ...
        <div className="overflow-auto pb-20">
            ...
            {nftInCart.map((nft) => (
              <div>
                <div className="flex flex-row items-center justify-between px-3" key={nft.id}>
                  ...
                  <Image className="w-16 h-16 rounded-xl" width={50} height={50} src={nft.image}/>
                  <div>
          ...
          <div className="px-3">
            <button className="w-full p-3 bg-blue-600 my-3 text-white rounded-xl hover:scale-105 shadow-lg hover:bg-blue-800 duration-200" onClick={() => (isLoggedIn ? "" : setLogin())}>
              Complete purchase
            </button>
          </div>
        </div>
